...
class BookTowApiController extends ApiBaseController
{
        ...
        try {
            ...
            foreach($truck_category_types as $type){
                $pricing_calculation[] = [
                    'id' => $type->id,
                    'name' => $type->name,
                    'pricing' => distance($request->gps_lat_from,$request->gps_lng_from,$request->gps_lat_to,$request->gps_lng_to,'K') * $type->price_per_km
                ];
            }
    ...
    public function find_tow(Request $request){
        ...
        try {
            ...
            $drivers = Driver::geofence($request->gps_lat_from, $request->gps_lng_from, ($request->radius) ? $request->radius : 5, ($request->radius) ? $request->radius : 5);
...
//        try {
            ...
            $drivers = Driver::geofence($request->gps_lat_from, $request->gps_lng_from, ($request->radius) ? $request->radius : 10, ($request->radius) ? $request->radius : 50);
